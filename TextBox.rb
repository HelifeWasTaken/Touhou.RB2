...
class TextBoxHandler
  ...
  def set_left_character(chara, flip=false)
  ...
  def set_right_character(chara, flip=false)
  ...
  def add_dialog_music(music)
    @dialog_music[Omega.min(0, @full_text.size - 1)] = music
    self
  end
  ...
  def start
    ...
    @dialog_music[0].play(true) if not @dialog_music[0].nil?
    self
  end
    ...
    if @boxes[@current_index].is_buffer_full or @boxes[@current_index].has_finished_to_write()
      if Game::is_just_pressed_ok
        ...
        if @boxes[@current_index].has_finished_to_write
          @current_index += 1
          @dialog_music[@current_index].play(true) if not @dialog_music[@current_index].nil?
          if finished and not Gosu::Song.current_song.nil?
...
class DialogList
  ...
  def draw
    @texts[0].draw if not @texts[0].nil?
  end
